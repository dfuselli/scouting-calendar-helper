...
GRUPPI = ["Anno", "Societa", "Stato", "Nome", "Osservatore"]
...
def load_csv(buffer) -> pd.DataFrame:
    ...
    df["NDS"] = (~df["Segnalato"].str.strip().str.lower().eq("true")).astype(
        int
    )  # 1 = visto senza segnalare
...
c3.metric("Segnalati", n_segnalati)
