...
def load_calendar_data_from_db(filter_next_7_days=True):
    ...
    if df.empty:
        st.warning(
            "Nessuna partita trovata nei prossimi 7 giorni."
            if filter_next_7_days
            else "Il calendario è vuoto."
        )
        return df
    else:
        df["ID"] = df.apply(
        ...
        df["_TimeSort"] = pd.to_datetime(
            df["Data"].dt.strftime("%Y-%m-%d") + " " + df["Ora"]
        )
        df["Time"] = df["_TimeSort"].dt.strftime("%d/%m %H:%M")
        df["Fascia"] = df.apply(
            lambda row: (
                f"{'🟡' if 'CSI' in row['Federazione'].upper() else '🔵'}{merge_categoria_federazione(row['Categoria'])}"
            ),
            axis=1,
        )
        df["Selezionato"] = False
        df = df.sort_values(by=["_TimeSort", "Casa"], ascending=[True, True]).drop(
            columns="_TimeSort"
        )
        return df
...
def aggregate_by_comune(df_dist, df_geo):
    df_agg = df_dist.groupby("Comune_casefold", as_index=False).agg(
    ...
    df_full["n_squadre"] = (
        pd.to_numeric(df_full["n_squadre"], errors="coerce").fillna(0).astype(int)
    )
